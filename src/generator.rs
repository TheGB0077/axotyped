...
pub fn generate(routes: &RouteCollection, config: &GeneratorConfig) -> String {
    ...
    // Collect and emit type imports from the bindings directory.
    // Types are exported via RouteCollection::export_types() which uses
    // ts-rs's export_all() to write individual .ts files including
